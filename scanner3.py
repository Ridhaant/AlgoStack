...
live_state.json structure:
{
  ...
  "merged_best": {...},
  "cross_scanner_best": {
    "NIFTY": {"best_x": 0.008, "pnl": 2400.0, "source": "scanner1"}
  },
...
from sweep_core import (
    ...
    IST, BROKERAGE_PER_SIDE, COMMODITY_SYMBOLS,
...
SCANNER_NAME  = "Scanner-3 WideDual (0.016–0.032)"
SCANNER_ID    = 3

# ── LOWER band: X ∈ [0.016000–0.024000] ──────────────────────────────────────
X_STEP        = 0.000001
...
# ── UPPER band: X ∈ (0.024000–0.032000] (dedupe the split boundary) ──────────
X_UPPER_MIN_RAW = 0.024000
X_UPPER_MIN   = X_UPPER_MIN_RAW + X_STEP
X_UPPER_MAX   = 0.032000
X_UPPER_NP    = gen_x_values(X_UPPER_MIN, X_UPPER_MAX, X_STEP)
N_UPPER       = len(X_UPPER_NP)

TOTAL_VARIATIONS = N_LOWER + N_UPPER
...
# ── Commodity ranges (same X logic as equity scanner-3) ─────────────────────
COMMODITY_LOWER: Dict[str, Tuple[float, float, int]] = {
    sym: (X_LOWER_MIN, X_LOWER_MAX, N_LOWER) for sym in COMMODITY_SYMBOLS
}
COMMODITY_UPPER: Dict[str, Tuple[float, float, int]] = {
    sym: (X_UPPER_MIN, X_UPPER_MAX, N_UPPER) for sym in COMMODITY_SYMBOLS
}
...
_s3cx = os.getenv("TELEGRAM_CHAT_IDS", "").strip()
TELEGRAM_CHATS = (
    [c.strip() for c in _s3cx.split(",") if c.strip()]
    if _s3cx
...
def _compute_merged_best(
    ...
    for sym in set(sweeps_lower) | set(sweeps_upper):
        ...
        dl = sl.dump_state(compact=True) if sl else {}
        du = su.dump_state(compact=True) if su else {}
    ...
    state: Dict[str, Any] = {
        ...
        "bands": {
            "lower": {
                ...
                "sweeps": {sym: sw.dump_state(compact=True) for sym, sw in sweeps_lower.items()},
            },
            "upper": {
                "x_min": X_UPPER_MIN, "x_max": X_UPPER_MAX, "n": N_UPPER,
                "sweeps": {sym: sw.dump_state(compact=True) for sym, sw in sweeps_upper.items()},
            },
        },
        "merged_best":        merged,
...
def main() -> None:
    ...
    for sym, pc, src in results:
        if pc is None:
            ...
            continue
        is_comm = False  # scanner3 is equity-only

        if is_comm:
            # Commodity scanner-3 uses the same X grid as equity scanner-3.
            xv_l = X_LOWER_NP
            xv_u = X_UPPER_NP
        ...
        with Live(
            ...
            while True:
                ...
                if prices:
                    ...
                    def _sweep_lower():
                        nonlocal eligible, missing_price, lower_updated
                        for sym, sw in sweeps_lower.items():
                    ...
                    merged = _compute_merged_best(sweeps_lower, sweeps_upper)
                    traded_symbols = sum(1 for m in merged.values() if int(m.get("trade_count", 0) or 0) > 0)
                    last_diag = {
                        "symbols_total": len(sweeps_lower),
                        "symbols_eligible": eligible,
                        "symbols_missing_price": missing_price,
                        "symbols_lower_updated": lower_updated,
                        "symbols_upper_updated": upper_updated,
                        "symbols_with_trades": traded_symbols,
                        "price_ticks_seen": int(store.ticks),
                        "last_sweep_ms": round(sweep_ms, 2),
                        "cross_sources": len(sources_used),
