...
# ── Commodity ranges per band (same X logic as equity scanner-2) ───────────
...
_s2cx = os.getenv("TELEGRAM_CHAT_IDS", "").strip()
TELEGRAM_CHATS = (
    [c.strip() for c in _s2cx.split(",") if c.strip()]
    if _s2cx
...
def main() -> None:
    ...
    with ThreadPoolExecutor(max_workers=min(20, len(equity_syms))) as ex:
        results = list(ex.map(_get_pc, equity_syms))
    ...
    for sym, pc, src in results:
        if pc is None:
            ...
            continue
        is_comm = False  # scanner2 is equity-only

        # Determine X arrays for each band
        if is_comm:
            # Commodity scanner-2 uses the same X grid as equity scanner-2.
            xv_lower = X_LOWER_NP
            xv_upper = X_UPPER_NP
