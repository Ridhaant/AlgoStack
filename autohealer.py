...
_ah_raw = os.getenv("TELEGRAM_CHAT_IDS", "").strip()
TG_CHATS    = (
    [c.strip() for c in _ah_raw.split(",") if c.strip()]
    if _ah_raw
...
# Hardcoded operation mode: run full stack under autohealer without extra toggles.
FORCE_FULL_STACK_ALWAYS = True
MAX_BACKOFF_S         = 300
TOO_MANY_RESTARTS_RESET_S = 900

# Process dependency graph (fail-safe startup/restart ordering).
PROCESS_DEPS: Dict[str, Tuple[str, ...]] = {
    "Scanner1": ("Algofinal",),
    "Scanner2": ("Algofinal",),
    "Scanner3": ("Algofinal",),
    "XOptimizer": ("Scanner1", "Scanner2", "Scanner3"),
    "BestXTrader": ("Algofinal", "XOptimizer"),
    "CommodityBestXTrader": ("CommodityEngine", "XOptimizer"),
    "CommScanner1": ("CommodityEngine",),
    "CommScanner2": ("CommodityEngine",),
    "CommScanner3": ("CommodityEngine",),
    "CryptoBestXTrader": ("CryptoEngine", "XOptimizer"),
    "CryptoScanner1": ("CryptoEngine",),
    "CryptoScanner2": ("CryptoEngine",),
    "CryptoScanner3": ("CryptoEngine",),
    "SLInspector": ("AlertMonitor",),
}
...
PROCESSES: List[dict] = [
    ...
    },
    {
        ...
        "env":         {
            ...
            # Reduce callback pressure and improve public tunnel responsiveness.
            "ALGOSTACK_TUNNEL_LITE": "1",
            # Give quick tunnel a bit more probe tolerance before rotation.
            "TRY_CLOUDFLARE_HTTP_FAILS": "8",
            # Higher waitress worker pool prevents "Task queue depth" backlog.
            "WAITRESS_THREADS": "24",
        },
        "port":        8055,
        "start_delay": 12,
    ...
    {
        "name":        "CommodityBestXTrader",
        "cmd":         [sys.executable, "best_x_trader_commodity.py"],
        "env":         {},
        "port":        None,
        "start_delay": 10,
        "auto_restart": True,
        ...
        "health_url":  None,
        "weekday_only": True,
        "description": "MCX Commodity Best-X paper trader (paper logs only)",
    },
    ...
    {
        "name":        "CryptoBestXTrader",
        "cmd":         [sys.executable, "best_x_trader_crypto.py"],
        "env":         {},
        "port":        None,
        "start_delay": 10,
        "auto_restart": True,
        ...
        "health_url":  None,
        "weekday_only": False,
        "description": "Crypto Best-X paper trader (paper logs only)",
    },
    {
        "name":        "CryptoScanner1",
    ...
    {
        "name":        "SLInspector",
        "cmd":         [sys.executable, "stoploss_inspector.py"],
        "env":         {},
        "port":        None,
        "start_delay": 20,
        "auto_restart": True,
        "critical":    False,
        "health_url":  None,
        "description": "Stoploss insights report generator",
        "weekday_only": False,
    },
    {
        "name":        "SentinelAPI",
        "cmd":         [sys.executable, "-m", "uvicorn", "app.main:app", "--host", "127.0.0.1", "--port", "8099", "--app-dir", "sentinel-portfolio"],
        "env":         {},
        "port":        8099,
        "start_delay": 20,
        "auto_restart": True,
        ...
        "description": "Cybersecurity wallet API",
        "weekday_only": False,
    },
]
...
def _resolve_profile(cli_profile: Optional[str], lite_flag: bool) -> str:
    # Hardcoded default: full watchdog-managed stack.
    if lite_flag:
        return "render-lite"
    if cli_profile:
        return cli_profile
    ...
    def start(self) -> None:
        ...
        if self.name == "UnifiedDash":
            if _is_render_host():
                ...
                env.setdefault("ALGOSTACK_ADMIN_EMAILS", "")
...
class Watchdog:
    def __init__(self, managed: List[ManagedProcess]) -> None:
        ...
        self._by_name = {m.name: m for m in managed}

    def _deps_ready(self, mp: ManagedProcess) -> bool:
        deps = PROCESS_DEPS.get(mp.name, ())
        if not deps:
            return True
        for dn in deps:
            dmp = self._by_name.get(dn)
            if dmp is None:
                continue
            if not dmp.is_alive():
                return False
            if dmp.status not in ("running", "starting"):
                return False
        return True
    ...
    def start_all(self) -> None:
        ...
        force_full = FORCE_FULL_STACK_ALWAYS
        if not is_td:
            ...
            if force_full:
                log.warning("  AUTOHEALER_FORCE_FULL_STACK=1 — starting FULL stack override.")
            else:
                log.warning("  NSE/MCX CLOSED — starting light stack (Dashboard + Crypto).")
        ...
        for mp in self.managed:
            # On NSE holidays/weekends: keep the site responsive by not launching
            # heavy equity/commodity scanners/optimizers that cannot trade anyway.
            if not is_td and not force_full:
                if mp.name not in (
                    "UnifiedDash",
                    "CryptoEngine",
                    "CryptoBestXTrader",
            ...
            if d > 0:
                ...
                time.sleep(d)
            if not self._deps_ready(mp):
                mp.status = "waiting_deps"
                log.info("[%s] Waiting for dependencies: %s", mp.name, ", ".join(PROCESS_DEPS.get(mp.name, ())))
                continue
            mp.start()
            time.sleep(0.5)
    ...
    def _check_one(self, mp: ManagedProcess) -> None:
        if mp.status == "stopped":
            return
        # Allow recovery after cooldown from restart-loop state.
        if mp.status == "too_many_restarts":
            if (time.monotonic() - (mp.last_restart_ts or 0.0)) >= TOO_MANY_RESTARTS_RESET_S:
                log.warning("[%s] Cooldown elapsed — retrying process recovery.", mp.name)
        ...
        if mp.is_alive():
            # Health fail-safe: tolerate a few probe misses before classifying degraded.
            if mp.cfg.get("health_url"):
                if mp.check_health():
                    mp.health_fail_count = 0
                    mp.status = "running"
                else:
                    mp.health_fail_count += 1
                    mp.status = "degraded" if mp.health_fail_count < 3 else "crashed"
                    if mp.status == "degraded":
                        return
            else:
                mp.status = "running"
            return
        ...
        if mp.restarts_last_hour() >= MAX_RESTARTS_PER_HOUR:
            if mp.status != "too_many_restarts":
                ...
                mp.last_restart_ts = time.monotonic()
            return

        # Exponential backoff for repeated failures.
        backoff = min(MAX_BACKOFF_S, RESTART_COOLDOWN_S * (2 ** max(0, mp.total_restarts - 1)))
        if mp.restart_times and (time.monotonic() - mp.restart_times[-1]) < backoff:
            return
        ...
        if weekday_only and not _is_trading_day() and not FORCE_FULL_STACK_ALWAYS:
            if mp.status != "market_closed":
                mp.status = "market_closed"
...
def _live_display(
    ...
    _STATUS = {
        ...
        "waiting_deps":      ("blue",   "⏸ Waiting deps"),
        "crashed":           ("red",    "✗ Crashed"),
        "stopped":           ("dim",    "○ Stopped"),
