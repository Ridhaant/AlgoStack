...
# ngrok — supply via environment or ngrok_token.txt (never commit tokens)
NGROK_AUTHTOKEN_EQUITY = os.getenv("NGROK_AUTHTOKEN_EQUITY", os.getenv("NGROK_AUTHTOKEN", ""))
NGROK_AUTHTOKEN_COMMODITY = os.getenv("NGROK_AUTHTOKEN_COMMODITY", os.getenv("NGROK_AUTHTOKEN", ""))
NGROK_AUTHTOKEN_DEFAULT = NGROK_AUTHTOKEN_EQUITY
                    ...
                    for idx, row in sub.iterrows():
                        ...
                        if _is_entry_event_type(event):
                            ...
                            if entry_price and qty_open > 0:
                                ...
                                # v10.10 FIX: Prefer the engine's stored P&L fields when available.
                                # The event stream already persists authoritative gross/net for exits
                                # (including RETREAT), and pairing-based reconstruction can drift.
                                stored_gross = row.get('gross_pnl', None)
                                if stored_gross is None:
                                    stored_gross = row.get('gross', None)
                                stored_net = row.get('net_pnl', None)
                                if stored_net is None:
                                    stored_net = row.get('net', None)

                                pnl = None
                                net_pnl = None
                                brokerage = None
                                if stored_gross is not None and stored_net is not None:
                                    try:
                                        pnl = float(stored_gross)
                                        net_pnl = float(stored_net)
                                        brokerage = float(row.get('brokerage', pnl - net_pnl))
                                    except Exception:
                                        pnl = None
                                        net_pnl = None
                                        brokerage = None

                                if pnl is None or net_pnl is None:
                                    brokerage = BROKERAGE_FLAT_PER_SIDE + BROKERAGE_FLAT_PER_SIDE
                                    # Retreat exits have a special P&L definition:
                                    # gross is always positive and equals lv.x * 0.25 * qty (not based on fill price).
                                    if event in ('BUY_RETREAT_25PCT', 'SELL_RETREAT_25PCT'):
...
# Telegram — tokens and chat IDs from .env only (public repo safe)
TELEGRAM_BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN", "")
_TELEGRAM_IDS_RAW = os.getenv("TELEGRAM_CHAT_IDS", "").strip()
if _TELEGRAM_IDS_RAW:
    TELEGRAM_CHAT_IDS_ALL = [c.strip() for c in _TELEGRAM_IDS_RAW.split(",") if c.strip()]
    TELEGRAM_CHAT_ID = TELEGRAM_CHAT_IDS_ALL[0] if TELEGRAM_CHAT_IDS_ALL else ""
    TELEGRAM_EXTRA_CHAT_IDS = TELEGRAM_CHAT_IDS_ALL[1:]
else:
    TELEGRAM_CHAT_ID = os.getenv("TELEGRAM_CHAT_ID", "")
    _extra_raw = os.getenv("TELEGRAM_EXTRA_CHAT_IDS", "").strip()
    TELEGRAM_EXTRA_CHAT_IDS = [c.strip() for c in _extra_raw.split(",") if c.strip()] if _extra_raw else []
    TELEGRAM_CHAT_IDS_ALL = [
        cid for cid in [TELEGRAM_CHAT_ID, *TELEGRAM_EXTRA_CHAT_IDS] if cid
    ]
TELEGRAM_COMMODITY_BOT_TOKEN = os.getenv("TELEGRAM_COMMODITY_BOT_TOKEN", "")
...
def maybe_alert_once(state: SymbolState, key: str, text: str) -> None:
    ...
    if state.entry_time is not None:
        try:
            trade_scope = state.entry_time.astimezone(IST).strftime("%Y%m%d%H%M%S")
        except Exception:
            trade_scope = state.entry_time.strftime("%Y%m%d%H%M%S")
    scoped_key = f"{state.symbol}:{trade_scope}:{key}"
    if scoped_key in state.sent_events:
        logger.info(
            "Alert dedupe suppressed symbol=%s key=%s side=%s in_position=%s",
            state.symbol,
            scoped_key,
            state.side,
            state.in_position,
    ...
    if alerts_allowed(now_local, symbol=state.symbol):
        ...
        state.sent_events.add(scoped_key)
...
def _open_entry(
    ...
    *,
    ...
    if dyn_qty <= 0:
        ...
        return
    # De-dup alerts only within the active trade instance.
    state.sent_events.clear()
...
class UdpPricePublisher:
    ...
    def publish(self, prices: dict, ts: 'datetime', min_interval_s: float = 0.0) -> None:
        """Merge-write prices to shared JSON file. Never blocks the trading loop.

        Important: other engines (CommodityEngine/CryptoEngine/PriceService) also
        merge their own sections into this file. Never overwrite those keys.
        """
        try:
            now_t = time.time()
            if min_interval_s and (now_t - self._last_write_t) < float(min_interval_s):
                return
            # Read existing so we preserve commodity_prices / crypto_prices.
            existing: dict = {}
            if os.path.exists(LIVE_PRICES_FILE):
                try:
                    with open(LIVE_PRICES_FILE, "r", encoding="utf-8") as fh:
            ...
            # Backward compat: "prices" should be the merged view.
            merged: dict = {}
            ...
            data = json.dumps(existing, separators=(",", ":"), ensure_ascii=False)
