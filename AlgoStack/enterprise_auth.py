...
SQLite users/orgs, append-only audit log, multi-tenant hooks.
...
def _verify_password(row: sqlite3.Row, password: str) -> bool:
...
def _consume_backup_code(row: sqlite3.Row, code: str) -> bool:
    from werkzeug.security import check_password_hash
...
def _lookup_valid_reset_token(raw_token: str) -> Optional[sqlite3.Row]:
