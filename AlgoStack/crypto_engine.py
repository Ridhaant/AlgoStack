...
# CRYPTO_X updated to 0.008 (was 0.008575) — tighter, cleaner anchor constant
# ═══════════════════════════════════════════════════════════════════════
"""
...
def _process_price(sym: str, price: float, ts: datetime) -> None:
    ...
    def _close(exit_px: float, reason: str):
        ...
        if (reason_log or reason).startswith("T") and "_HIT" in (reason_log or reason):
            _apply_crypto_l_reanchor(sym, float(exit_px_log))
        elif (reason_log or reason) == "SL_HIT" and L_sl_for_reanchor is not None:
